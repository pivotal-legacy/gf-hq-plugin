 ...
 * entered by user.
 */
public class SQLFirePlatformDetector extends PlatformDetector {

    private static final Log log =
        LogFactory.getLog(SQLFirePlatformDetector.class);
    ...
    throws PluginException {
        ...
        if(log.isDebugEnabled()) {
            log.debug("Detecting SQLfire Distributed System");
        ...
        if(pids.length == 0) {
            if(log.isDebugEnabled())
                log.debug("No SQLfire JMX Agent processes detected");
    ...
    private static long[] getAgentPids() {
        try {
            long pids[] = ProcessFinder.find(getSigar(), "Args.*.eq=com.vmware.sqlfire.tools.SqlfAgentLauncher");
            return pids;

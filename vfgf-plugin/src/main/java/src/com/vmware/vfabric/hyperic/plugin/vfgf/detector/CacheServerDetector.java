...
import com.vmware.vfabric.hyperic.plugin.vfgf.detector.MemberDetector.StatType;
...
public class CacheServerDetector extends MemberDetector {
    ...
    private final static Log log =
        LogFactory.getLog(CacheServerDetector.class);

    @Override
    protected boolean hasCorrectRoles(int mask) {
        if(log.isDebugEnabled()) log.debug(GFUtils.roleMaskToDebugString(mask));
        return ((mask & GFMXConstants.MEMBER_ROLE_CACHESERVER) == GFMXConstants.MEMBER_ROLE_CACHESERVER &&
    ...
    protected StatType[] filterSupportedStats(Object[][] statObjects, ConfigResponse config) {
        ...
        for (int i = 0; i < statObjects.length; i++) {
            ...
            } else if(name.equals("statSampler")) {
                stats.add(new StatType("Statistics Sampler", o));

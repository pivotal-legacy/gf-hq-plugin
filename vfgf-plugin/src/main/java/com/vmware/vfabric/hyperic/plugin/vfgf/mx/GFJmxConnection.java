...
public class GFJmxConnection extends AbstractGFJmxConnection {
    ...
    public Map<String, String[]> getCacheVmsAttributes(){
        ...
        for (ObjectName objectName : mServers) {
            try {
                ...
                log.debug("[getCacheVmsAttributes] id: " + objectName.getCanonicalName());
                log.debug("[getCacheVmsAttributes] cwd: " + cwd);
                log.debug("[getCacheVmsAttributes] name: " + name);
                log.debug("[getCacheVmsAttributes] host: " + host);					

            } catch (Exception e) {
                log.debug("[getCacheVmsAttributes] " + e.getMessage(), e);
            }
        }
    ...
    public MemberInfo[] getCacheVmMembers(){
        ...
        for (ObjectName objectName : mServers) {
            try {
                ...
                log.info("[getCacheVmMembers] Unable to request cache members.", e);
            }
        }
    ...
    public MemberInfo[] getSystemMemberApplications(){
        ...
        for (ObjectName objectName : mServers) {
            try {
                ...
                log.debug("[getSystemMemberApplications] id: " + objectName.getCanonicalName());
                log.debug("[getSystemMemberApplications] name: " + name);
                log.debug("[getSystemMemberApplications] host: " + host);					

            } catch (Exception e) {
                log.info("[getSystemMemberApplications] Unable to request cache members.", e);
            }
        }
    ...
    public Map<ObjectName, String[]> getCacheVmAttributesAsObjectName(){
        ...
        for (ObjectName objectName : mServers) {
            try {
                ...
                log.debug("[getCacheVmAttributesAsObjectName] id: " + objectName.getCanonicalName());
                log.debug("[getCacheVmAttributesAsObjectName] cwd: " + cwd);
                log.debug("[getCacheVmAttributesAsObjectName] name: " + name);
                log.debug("[getCacheVmAttributesAsObjectName] host: " + host);					
            } catch (Exception e) {
                log.debug("[getCacheVmAttributesAsObjectName] " + e.getMessage(), e);
            }
        }
        return map;
    }
        ...
        for (ObjectName objectName : stats) {
            try {
                ...
                if(type.equals("CacheServerStats")) { 
                    mask = mask | GFMXConstants.MEMBER_ROLE_CACHESERVER;
                } else if(type.endsWith("GatewayHubStatistics")) {
                    mask = mask | GFMXConstants.MEMBER_ROLE_GATEWAYHUB;
                }
            } catch (Exception e) {
            }            
        }
    ...
    public Object[][] getStatObjectsWithType(String gfid) throws PluginException {
        ...
        try {
            ...
            log.debug("[getStatObjectsWithType] " + e.getMessage(), e);
        }
